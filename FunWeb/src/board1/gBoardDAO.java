...
//      return con;
   ...
   public int deletePro(int num, String pass) {
      ...
      try {
         ...
         String sql = "select pass from board1 where num=?";
         ...
         if(rs.next()) {
            if(rs.getString("pass").equals(pass)) {
               sql = "delete from board1 where num=? and pass=?";
   ...
   public void insertBoard(gBoardBean bb) {
      ...
      try {
         ...
         //3단계 sql (insert) now()
          sql="insert into board1(num,name,pass,subject,content,readcount,date,file) values(?,?,?,?,?,?,now(),?)";
          pstmt=con.prepareStatement(sql);
   ...
   //getBoardList()
   public List<gBoardBean> getBoardList(int startRow,int pageSize) {
      Connection con=null;
      PreparedStatement pstmt=null;
      ResultSet rs=null;
      List<gBoardBean> boardList=new ArrayList<gBoardBean>();
      try {
         //1단계 드라이버 로더         //2단계 디비연결
         con=getConnection();
...
//          String sql="select * from board order by num desc";
   ...
   //getBoardList()
      public List<gBoardBean> getBoardList(int startRow,int pageSize, String search) {
         Connection con=null;
         PreparedStatement pstmt=null;
         ResultSet rs=null;
         List<gBoardBean> boardList=new ArrayList<gBoardBean>();
         try {
            ...
            con=getConnection();
             // 3단계 sql(select) 만들고 실행할 객체 생성
//             String sql="select * from board order by num desc";
            String sql="select * from board1 where subject like ? order by num desc limit ?,?";
             pstmt=con.prepareStatement(sql);
   ...
   public void updateBoard(gBoardBean bb) {
      ...
      try {
         ...
         con=getConnection();
         //3단계 sql update
         String sql="update board1 set name=?,subject=?,content=?,file=? where num=?";
          pstmt=con.prepareStatement(sql);
   ...
   //getBoardCount()
      public int getBoardCount(String search) {
         ...
         try {
            ...
            String sql="select count(*) from board1 where subject like ?";
            pstmt=con.prepareStatement(sql);
            pstmt.setString(1, "%"+search+"%");
            //4 rs = 실행
            rs=pstmt.executeQuery();
